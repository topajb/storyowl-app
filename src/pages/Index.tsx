...
const Index = () => {
  ...
  const handleFormSubmit = async (formData: MythologyStoryParams) => {
    ...
    setLoadingStep('Preparing to create your magical mythology story...');

    try {
      const story = await mythologyStoryService.generateMythologyStory(
        formData,
        (progress, step) => {
      ...
      console.error('Failed to generate mythology story:', error);
      setCurrentState('form');
    } finally {
  ...
  switch (currentState) {
    ...
    case 'form':
      return <MythologyStoryForm onSubmit={handleFormSubmit} onBack={handleBackHome} isLoading={isGenerating} />;
